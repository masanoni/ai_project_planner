            ...
            const rows: string[][] = jsonData.slice(1).map(rowArray => {
                ...
                rowArray.forEach((cell, index) => {
                    if (index < headers.length) {
                        fullRow[index] = cell !== null && cell !== undefined ? String(cell) : '';
                    }
                });
                return fullRow;
            });
                    ...
                    {(report.attachments || []).map(att => (
                        <div key={att.id} className="relative group border rounded-md overflow-hidden">
                            {att.type.startsWith('image/') ? (
                                ...
                                <a href={att.dataUrl} download={att.name} className="w-full h-24 bg-slate-100 flex flex-col items-center justify-center p-2 hover:bg-slate-200">
                                  <PaperClipIcon className="w-6 h-6 text-slate-500"/>
                                  <span className="text-xs text-slate-600 text-center break-all w-full mt-1" title={att.name}>{att.name}</span>
                                </a>
                            )}
                            <button onClick={() => handleRemoveAttachment(att.id)} className="absolute top-1 right-1 bg-black bg-opacity-50 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity">
